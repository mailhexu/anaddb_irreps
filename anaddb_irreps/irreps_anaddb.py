...
class IrRepsAnaddb(IrRepsEigen):
    ...
    Added helpers focus on a compact summary table that is easier to
    consume programmatically and via the CLI.
    """
    ...
    def get_summary_table(self):
        ...
        if not hasattr(self, "_freqs"):
            raise RuntimeError(
                "IrRepsAnaddb.run() must be called before get_summary_table()."
            )
...
def print_irreps(
    ...
    """Compute and print irreps for a single q-point in one function call.
    ...
    Args:
        ...
        degeneracy_tolerance: Frequency difference tolerance for degeneracy (default: 1e-4)
        log_level: Verbosity level (default: 0)
        show_verbose: If True, also print verbose phonopy-style output (default: False)

    Returns:
        IrRepsAnaddb: The computed irreps object for further programmatic access
